...
class BarX:
    def __init__(self, x_data, members=1, gwidth=1):
        """
        Calculate x-values for a bar graph

        x_data  : the location of the centers
        members : the number of bars in a group
        width   : the total width of a group
        """
        self.centers = _np.asarray(x_data)
        assert self.centers.size

        self.gwidth = float(gwidth)
        assert self.gwidth > 0

        self.members = int(members)
        assert self.members >= 1

        self.mwidth = self.gwidth / self.members

        self.L = []
        for i in range(self.members):
            self.L.append(self.centers - 0.5 * self.gwidth +
                i * self.mwidth)
        self.L = _np.array(self.L)
        self.C = self.L + 0.5 * self.mwidth
        self.R = self.L + 1.0 * self.mwidth

        self.xmin = _np.amin(self.centers) - self.gwidth
        self.xmax = _np.amax(self.centers) + self.gwidth
        self.xrange = (self.xmin, self.xmax)

    def where(self, where):
        values = list(self.centers)
        slicer = []
        for value in where:
            try:
    ...
    def x_values(self, i=0, which='L', slicer=None, where=None):
        """
        Get x-values for set i
        """
        if slicer and not where is None:
            raise RuntimeError('can not slice and index using where')

        if which.lower() in ['l', 'left']:
            if slicer:
                return self.L[i][slicer]
            if not where is None:
                slicer = self.where(where)
                return self.L[i][slicer]
            return self.L[i]

        if which.lower() in ['c', 'center']:
            if slicer:
                return self.C[i][slicer]
            if not where is None:
                slicer = self.where(where)
                return self.C[i][slicer]
            return self.C[i]

        if which.lower() in ['r', 'right']:
            if slicer:
                return self.R[i][slicer]
            if not where is None:
                slicer = self.where(where)
                return self.R[i][slicer]
            return self.R[i]

        raise RuntimeError('which must be l, c, or r')
