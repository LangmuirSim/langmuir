...
def get_arguments(args=None):
    ...
    parser.add_argument('--real', dest='real', type=int, metavar='int',
               default=None, help='iterations.real when --fresh or --equil ' +
               'are not set')
    ...
    parser.add_argument('--equil', dest='equil', type=int, metavar='int',
               default=None, help='override iterations.real for ' +
               'equilibrated runs')

    parser.add_argument('--print', dest='iprint', type=int, metavar='int', default=1000,
    ...
    parser.add_argument('--fmt', default='%s', type=str, help='value format string')

    parser.add_argument('--mode', dest='mode', type=str, metavar='str', default=None,
               choices=['gen', 'scan'], help='what to do (gen or scan)')

    parser.add_argument('--stub', dest='stub', default='run', type=str,
...
class Scanner:
    ...
    def load(self, opts):
        ...
        """
        self.checkpoint = langmuir.checkpoint.load(opts.template)
        ...
        if self.working == 'voltage.right':
            opts.fmt = '%+.1f'

        elif self.working == 'trap.potential':
            opts.fmt = '%+.3f'
    ...
    def generate(self, opts):
        ...
        stub = '%s_' % self.working + opts.fmt
        sims = [os.path.join(work, stub % value) for value in values]
        ...
        langmuir.common.command_script(paths, name='submit', stub=opts.stub)

    def scan(self, opts):

        stub = '%s_' % self.working + opts.fmt
        sims = [os.path.join(work, stub % value) for value in self.values]
        ...
        for this_index, (this_value, this_sim) in enumerate(zip(
            ...
            if os.path.exists(this_sim):
                last_chk = langmuir.checkpoint.load_last(this_sim)
                if last_chk['current.step'] < opts.equil:
                    ...
                    last_part = langmuir.find.parts(this_sim,
                                                    at_least_one=True)[-1]
                    this_path = os.path.join(this_sim, 'part.%d' % (
                        langmuir.regex.part(last_part) + 1))
                ...
                if last_sim and os.path.exists(last_sim):
                    #use last sim
                    this_chk = langmuir.checkpoint.load_last(last_sim)
