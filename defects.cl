...
__kernel void defects( __global int *iDevice, __global int *fDevice, __global int *dDevice, __global double *oDevice, int ndefects, int zDefect )
{
  ...
  for ( int load_number= 0; load_number < ndefects / get_local_size(0) + 1; load_number++ )
  {
   ...
   if ( load_id < ndefects )
   {
    sharedI[local_work_item_id] = dDevice[load_id];
   }
   else
   {
  ...
  if ( local_work_item_id == 0 )
  {
   ...
   oDevice[work_group_id] += ( -1 * zDefect * PREFACTOR * v );
  }
