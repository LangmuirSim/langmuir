...
\parameter{work.x}{int}{4}{%
    The number of x-threads in a 3D work group.  Only used for
        \texttt{output.coulomb}.
}
\parameter{work.y}{int}{4}{%
    The number of y-threads in a 3D work group.  Only used for
        \texttt{output.coulomb}.
}
\parameter{work.z}{int}{4}{%
    The number of z-threads in a 3D work group.  Only used for
        \texttt{output.coulomb}.
}
\parameter{work.size}{int}{256}{%
    The number of threads in a 1D work group.
}
...
\parameter{opencl.device.id}{int}{0}{%
    The id of the GPU to use when more than one is present.
}
\parameter{max.threads}{int}{-1}{%
    The max number of CPU threads allowed.
    If < 0, then use all available threads.
}
\tabucline[1pt]{-}
\end{tabu}
